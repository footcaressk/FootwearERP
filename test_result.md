...
metadata:
  created_by: "main_agent"
  version: "bugfix-component-bulk-matrix-topup"
  test_sequence: 9
  run_ui: false
...
backend:
  - task: "Bug fix — /api/components/bulk-matrix silently dropped Add-Stock qty for existing rows"
    ...
    status_history:
        - working: "NA"
          agent: "main"
          comment: "ROOT CAUSE: POST /api/components/bulk-matrix caught DuplicateKeyError on existing (component_code, color, size) rows and silently marked them 'exists' — completely ignoring the opening_qty the user entered. When the 'Add Stock in Bulk' drawer opened in EXTEND mode for a component that already has (color, size) rows (e.g. Ginger with Silver × 36-41), the drawer prefilled those cells, the user entered quantities, saved — and the backend threw them away. Symptom: user's screenshot shows the Ginger component with 0 stock across all sizes despite repeated Add-Stock attempts. FIX: in the DuplicateKeyError branch, when opening_qty > 0 look up the existing component_master row and record a `purchase_in` movement via _record_component_movement() with reference_type='bulk_matrix_topup', notes='Stock added via bulk matrix (extend mode)'. Increment a new `updated` counter. Return {created, updated, skipped, results}. `results[]` gets status='stock_added' with qty for topped-up rows. Frontend also updated: (1) useEffect in ComponentBulkDrawer.jsx now also fetches current_stock into existingStock map; (2) in extend mode, each cell shows 'was N → N+addQty' hint above the input (grey small text); (3) header changes from 'Enter Opening Quantities' to 'Enter Stock to Add (Δ Qty)' with an italic disclaimer 'Quantities you enter are ADDED to the current stock via a purchase-in movement'; (4) result banner reports 'New rows: X · Stock added to existing: Y · Skipped (existing, 0 qty): Z'."
        - working: true
          agent: "testing"
